...
5. **recovery 成功销账（验收关键门 R21-B5）**：
   - 构造"拾取发生在 recovery 启动之前"的真实时序：让 Bob 走到矿石原位上方的掉落物处，
     vanilla 自动拾取先完成（背包 raw_iron 0 → 3），此刻机会仍是 pending；
   ...
   - **未重新挖掘**：全程 `mine_start`/`mine_complete` 计数 0，ore 位保持 air。
...
- executions：`eb436008`（pending failed）、`cc52989c`（recovery completed）
- 日志：`server-r21-live-rerun.log`（`mode=pickup_recovery` 行、无 mine_start）
- registry：`registry-final-state.json`（复验轮结束时快照）
