# -*- coding: utf-8 -*-
"""LIVE-R11-1 part A3: 给镐使 C 变 ACTIONABLE 后再派发+停服 -> 干净 SUSPENDED。

前两次失败教训: (1)脚齐高矿到点即 no_reachable_work_pose; (2)无镐 -> 机会 BLOCKED,
mine_opportunity 派发 62ms 内类型化拒绝。本次: 悬空齐胸高矿 + iron_pickaxe + 2 秒停服。
"""
...
r2 = gl.call("POST", f"/v1/graphs/{graph_id}/run-next", lease, None,
             {"X-Request-Id": f"r11-l1c-{int(time.time()*1000)}"})
