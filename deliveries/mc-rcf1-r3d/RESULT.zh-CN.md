...
D 切片与非计分链已在本部署重跑,重跑记录为交付证据;首跑(旧部署
jar=d6ce2375)的 D05/D06 存档于 d-slices/first-run-archive/——其中
骷髅击杀翻会话的 session-lost 实证仅存于首跑):**
...
- 非计分确认:采木→板→棍→台→木镐→采石→石镐→再木镐全链首跑 67.6s/
  本部署重跑 235s(R3C 同链 200s+,均 fail_at=null);进入+连续
  封口 6s(两轮均 consecutive=true)。
