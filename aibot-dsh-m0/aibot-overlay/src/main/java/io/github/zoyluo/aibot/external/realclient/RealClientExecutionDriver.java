...
/** Minimal real-player actuator: chat, short goto and one server-validated opportunity mine. */
public final class RealClientExecutionDriver implements PhysicalExecutionDriver {
    public static final Set<String> OPERATIONS=Set.of("say","goto","mine_opportunity");
    ...
    private BodyBackend.Handle startGoto(
        ...
        if(!transport.sendCommand(
            ...
            throw new BridgeFault(503,"real_client_command_queue_unavailable");
        return ()->remoteSnapshot(request.executionId(),startedAt,()->{
            ServerPlayerEntity current=body.get();
            if(current==null)return null;
            if(current.getPos().squaredDistanceTo(target.toCenterPos())<=6.25D)
    ...
    private BodyBackend.Handle startMine(
        ...
        if(!transport.sendCommand(
            ...
            throw new BridgeFault(503,"real_client_command_queue_unavailable");
        return ()->mineSnapshot(
                request.executionId(),startedAt,opportunity,baseline);
    }
    ...
    private BodyBackend.Snapshot mineSnapshot(
        ...
        // Inventory gain alone is not causally sufficient: another actor could have handed Bob the
        // same item while the target ore still exists. Success requires BOTH the exact target cell
        // to be gone and the expected inventory family to have increased since dispatch.
        ...
        if(remote!=null && java.util.Set.of(
                "failed","cancelled","outcome_unknown").contains(remote.state())) {
    ...
    @Override public void cancel(String reason) {
        onThread();
        if(!activeExecution.isBlank())
            transport.sendControl(activeExecution,"cancel",reason==null?"external_cancel":reason);
    }
    ...
    private static int selectSuitableHotbar(ServerPlayerEntity player,net.minecraft.block.BlockState state) {
